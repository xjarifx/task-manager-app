      ...
      <div className="bg-[var(--card)] rounded-2xl border border-[var(--border)] shadow-sm">
        ...
        {error && (
          <div className="mx-5 mb-3 p-3 bg-blue-50 text-blue-700 border border-blue-200 rounded-lg flex justify-between items-center">
            <span>{error}</span>
            <button onClick={() => setError(null)} className="text-blue-400 hover:text-blue-600 font-bold">
              &times;
            </button>
          </div>
        )}
                ...
                {editingId === task.id ? (
                  ...
                  <div className="flex items-center gap-3 px-5 py-3 hover:bg-slate-50 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className={`text-sm font-medium leading-snug ${task.status === "DONE" ? "line-through text-[var(--muted)]" : "text-[var(--foreground)]"}`}>
                        {task.title}
                      </h3>
                      {task.description && (
                    ...
                    {/* Star */}
                    <button
                      onClick={() => setStatus(task, task.status === "IN_PROGRESS" ? "TODO" : "IN_PROGRESS")}
                      title={task.status === "IN_PROGRESS" ? "Remove star" : "Star task"}
                    ...
                    >
                      <svg className="w-4 h-4" viewBox="0 0 24 24" fill={task.status === "IN_PROGRESS" ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2">
